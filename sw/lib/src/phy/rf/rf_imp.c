...
{
  bool wait_gain_thread = false;
  // Stop gain thread
  pthread_mutex_lock(&rf->mutex);
  if (rf->thread_gain_run) {
    rf->thread_gain_run = false;
    wait_gain_thread = true;
  }
  pthread_mutex_unlock(&rf->mutex);
  pthread_cond_signal(&rf->cond);
  if (wait_gain_thread) {
...
static int load_plugin(srsran_rf_plugin_t* rf_plugin)
{
  ...
  if (rf_plugin->dl_handle == NULL) {
    ...
    printf("Failed to load RF plugin %s: %s", rf_plugin->plugin_name, dlerror());
    rf_plugin->rf_api = NULL;
    return SRSRAN_SUCCESS;
  }
  ...
  if ((err = dlerror()) != NULL) {
    printf("Error loading symbol '%s': %s", "register_plugin", err);
    goto clean_exit;
  }
  ...
  if (ret != SRSRAN_SUCCESS) {
    printf("Failed to register RF API for plugin %s", rf_plugin->plugin_name);
    goto clean_exit;
  }
  return SRSRAN_SUCCESS;
clean_exit:
