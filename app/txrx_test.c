...
int main(int argc, char** argv)
{
  ...
  if (input_filename) {
    ...
    for (int i = 0; i < flen - time_adv_samples; i++) {
      tx_buffer[i + time_adv_samples] = 0.98 * cexpf(_Complex_I * 2 * M_PI * tone_offset_hz * ((float)i / (float)srate));
    }
  ...
  while (nframe < nof_frames) {
    ...
    nframe++;
    if (nframe == 1 || nframe == 4 || nframe == 9) {
      srsran_timestamp_add(&tstamp, 0, 4e-3 - time_adv_sec);
  ...
  srsran_vec_save_file(output_filename, rx_buffer, flen * nof_frames * sizeof(cf_t));
